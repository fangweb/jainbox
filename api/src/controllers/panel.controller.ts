...
const validatePage = (request, response, next) => {
  ...
  if (isNaN(page)) {
    return next(new HttpError({ status: 401, message: CouldNotProcessRequest }));
  }
...
export class PanelController extends BaseController {
  ...
  private getInboxMessages: RequestHandler = async (request, response, next) => {
    const { tokenPayload, page } = response.locals;
    const [offset, limit] = getOffsetLimit(page);

    try {
      const result = await PanelRepository.getInboxMessages({ usernameId: tokenPayload.username_id, offset, limit });
  ...
  private getSentMessages: RequestHandler = async (request, response, next) => {
    ...
    try {
      const result = await PanelRepository.getSentMessages({ usernameId: tokenPayload.username_id, offset, limit });
      response.json(result);
    } catch (error) {
      next(new HttpError({ status: 409, message: error.message }));
    }
  ...
  private putMessageIntoInbox: RequestHandler = async (request, response, next) => {
    const { tokenPayload } = response.locals;
    const { message_id } = request.body;
    
    try {
      const result = PanelRepository.putMessageIntoInbox({ usernameId: tokenPayload.username_id, messageId: message_id });
      response.json(result);
    } catch (error) {
      next(new HttpError({ status: 409, message: error.message }));
    }
  ...
  private getTrashedMessages: RequestHandler = async (request, response, next) => {
    const { tokenPayload, page } = response.locals;
    const [offset, limit] = getOffsetLimit(page);

    try {
      const result = await PanelRepository.getTrashedMessages({ usernameId: tokenPayload.username_id, offset, limit });
      response.json(result);
    } catch (error) {
      next(new HttpError({ status: 409, message: error.message }));
    }
  ...
  private softDeleteMessage: RequestHandler = async (request, response, next) => {
    const { tokenPayload } = response.locals;
    const { message_id } = request.body;
    
    try {
      const result = PanelRepository.softDeleteMessage({ usernameId: tokenPayload.username_id, messageId: message_id });
      response.json(result);
    } catch (error) {
      next(new HttpError({ status: 409, message: error.message }));
    } 
